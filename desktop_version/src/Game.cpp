...
bool GetButtonFromString(const char *pText, SDL_GameControllerButton *button) {
	if (strcasecmp(pText, "BACK") == 0) {
		*button = SDL_CONTROLLER_BUTTON_BACK;
		return true;
	}
	if (strcasecmp(pText, "1") == 0) {
		*button = SDL_CONTROLLER_BUTTON_B;;
		return true;
	}
	if (strcasecmp(pText, "GUIDE") == 0) {
		*button = SDL_CONTROLLER_BUTTON_GUIDE;
		return true;
	}
	if (strcasecmp(pText, "START") == 0) {
		*button = SDL_CONTROLLER_BUTTON_START;
		return true;
	}
	if (strcasecmp(pText, "LS") == 0) {
		*button = SDL_CONTROLLER_BUTTON_LEFTSTICK;
		return true;
	}
	if (strcasecmp(pText, "RS") == 0) {
		*button = SDL_CONTROLLER_BUTTON_RIGHTSTICK;
		return true;
	}
	if (strcasecmp(pText, "LB") == 0) {
...
void Game::loadcustomlevelstats() {
	...
	if (!customlevelstatsloaded) {
		...
		if (!doc.LoadFile()) {
			...
			for (pElem = hRoot.FirstChild(
					"Data").FirstChild().Element(); pElem; pElem = pElem->NextSiblingElement()) {
				std::string pKey(pElem->Value());
				const char *pText = pElem->GetText();
				if (pText == NULL) {
					pText = "";
            ...
            */
			...
			case 1001:
				...
				if (dwgfx.flipmode) {
					...
					if (map.custommode) {
						dwgfx.createtextbox(
								" " + help.number(trinkets) + " out of " + help.number(map.customtrinkets) + " ", 50,
								65, 174, 174, 174);
						dwgfx.textboxcenterx();
					} else {
			...
			case 1011:
				...
				if (dwgfx.flipmode) {
					...
					if (int(map.customcrewmates - crewmates) == 0) {
						...
						dwgfx.createtextbox(
								"    " + help.number(int(map.customcrewmates - crewmates)) + " remains    ", 50, 135,
								174, 174, 174);
					} else {
						dwgfx.createtextbox(
								"     " + help.number(int(map.customcrewmates - crewmates)) + " remain    ", 50, 135,
								174, 174, 174);
					}
					dwgfx.textboxcenterx();
...
std::string Game::partimestring(UtilityClass &help) {
	...
	tempstring = "";
	if (timetrialpar >= 60) {
		tempstring = help.twodigits(int((timetrialpar - (timetrialpar % 60)) / 60)) + ":" +
		...
		if (ed.ListOfMetaData.size() == 0) {
			...
			for (int i = 0; i < (int) ed.ListOfMetaData.size(); i++) // FIXME: int/size_t! -flibit
			{
				if (i >= levelpage * 8 && i < (levelpage * 8) + 8) {
					...
					std::transform(menuoptions[tcount].begin(), menuoptions[tcount].end(),
								   menuoptions[tcount].begin(), ::tolower);
					tcount++;
				}
			}
